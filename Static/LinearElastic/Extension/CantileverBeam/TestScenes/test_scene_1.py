...
class TestScene():
    ...
    def createScene(self,rootNode, cs_param, param_idx, param, caseStudy_path):
        ...
        simulation.addObject('EulerImplicitSolver',firstOrder = False, rayleighMass = 0.1, rayleighStiffness = 0.1)
        ...
        # box2.init()
        beam.addObject('ConstantForceField', indices='@box2.indices', totalForce = [F,0.0,0.0])
...
class ErrorEvaluation(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self,event):
        ...
        if self.iter>=self.Niter-2 and not self.flag:
            ...
            mean_pos = self.tip_mo.position.value[0]
            self.disp_z = abs(self.pos_z_init-mean_pos[0])
